...
class SearchHistoryRepository(
     ...
     */
    fun getAllSearches(): Flow<Search> = searchDao
        .findAll().map { it.toDomain() }

    /**
     * Get search from history by query.
     *
    ...
    /**
     * Get existing search from history by internal id.
     *
     * @return the domain [Search] model.
     */
    suspend fun getSearchById(id: Int): Search? = searchDao.findById(id)?.toDomain()
